...
//
// Purpose:			Replacement for the medigun which also provides movement abilities
//					
//
// $Workfile:     $
// $Date:         $
...
#include "tf_weapon_medigloves.h"
...
// Buff ranges
ConVar weapon_medigloves_damage_modifier( "weapon_medigloves_damage_modifier", "1.5", FCVAR_CHEAT | FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY, "Scales the damage a player does while being healed with the medigloves." );
ConVar weapon_medigloves_construction_rate( "weapon_medigloves_construction_rate", "10", FCVAR_CHEAT | FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY, "Constructing object health healed per second by the medigloves." );
ConVar weapon_medigloves_charge_rate( "weapon_medigloves_charge_rate", "15", FCVAR_CHEAT | FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY, "Amount of time healing it takes to fully charge the medigloves." );
ConVar weapon_medigloves_passive_charge_rate( "weapon_medigloves_passive_charge_rate", "20", FCVAR_CHEAT | FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY, "Amount of time without healing it takes to fully charge the medigloves." );
ConVar weapon_medigloves_chargerelease_rate( "weapon_medigloves_chargerelease_rate", "8", FCVAR_CHEAT | FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY, "Amount of time it takes the a full charge of the medigloves to be released." );
ConVar weapon_medigloves_blast_num_chunks( "weapon_medigloves_blast_num_chunks", "4", FCVAR_CHEAT | FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY, "How many uber bar chunks the blast gloves have." );
ConVar weapon_medigloves_jump_num_chunks( "weapon_medigloves_jump_num_chunks", "6", FCVAR_CHEAT | FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY, "How many uber bar chunks the jump gloves have." );
ConVar weapon_medigloves_teleport_num_chunks( "weapon_medigloves_teleport_num_chunks", "3", FCVAR_CHEAT | FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY, "How many uber bar chunks the teleport gloves have." );
ConVar weapon_medigloves_grapple_num_chunks( "weapon_medigloves_grapple_num_chunks", "4", FCVAR_CHEAT | FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY, "How many uber bar chunks the grapple gloves have." );

#if defined (CLIENT_DLL)
ConVar tf_medigloves_autoheal( "tf_medigloves_autoheal", "1", FCVAR_CLIENTDLL | FCVAR_ARCHIVE | FCVAR_USERINFO, "Setting this to 1 will cause the Medigloves's primary attack to be a toggle instead of needing to be held down." );
#endif

#if !defined (CLIENT_DLL)
ConVar tf_medigloves_lagcomp(  "tf_medigloves_lagcomp", "1", FCVAR_DEVELOPMENTONLY );
#endif

static const char *s_pszMediglovesHealTargetThink = "MediglovesHealTargetThink";

extern ConVar tf_invuln_time;

/*	Already defined in tf_weapon_medigun.cpp
#ifdef CLIENT_DLL
//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
...
{
	CWeaponMedigloves *pMedigloves = ((CWeaponMedigloves*)(pStruct));
	if ( pMedigloves != NULL )
	{
		pMedigloves->ForceHealingTargetUpdate();
	}
...
LINK_ENTITY_TO_CLASS( tf_weapon_medigloves, CWeaponMedigloves );
PRECACHE_WEAPON_REGISTER( tf_weapon_medigloves );

IMPLEMENT_NETWORKCLASS_ALIASED( WeaponMedigloves, DT_WeaponMedigloves )
...
//-----------------------------------------------------------------------------
BEGIN_NETWORK_TABLE_NOBASE( CWeaponMedigloves, DT_LocalTFWeaponMediglovesData )
#if defined( CLIENT_DLL )
...
//-----------------------------------------------------------------------------
BEGIN_NETWORK_TABLE_NOBASE( CWeaponMedigloves, DT_TFWeaponMediglovesDataNonLocal )
#if defined( CLIENT_DLL )
RecvPropFloat( RECVINFO(m_flChargeLevel) ),
#else
...
//-----------------------------------------------------------------------------
BEGIN_NETWORK_TABLE( CWeaponMedigloves, DT_WeaponMedigloves )
#if !defined( CLIENT_DLL )
//	SendPropFloat( SENDINFO(m_flChargeLevel), 0, SPROP_NOSCALE | SPROP_CHANGES_OFTEN ),
	...
	SendPropDataTable("LocalTFWeaponMediglovesData", 0, &REFERENCE_SEND_TABLE(DT_LocalTFWeaponMediglovesData), SendProxy_SendLocalWeaponDataTable ),
	SendPropDataTable("NonLocalTFWeaponMediglovesData", 0, &REFERENCE_SEND_TABLE(DT_TFWeaponMediglovesDataNonLocal), SendProxy_SendNonLocalWeaponDataTable ),
#else
//	RecvPropFloat( RECVINFO(m_flChargeLevel) ),
	...
	RecvPropDataTable("LocalTFWeaponMediglovesData", 0, 0, &REFERENCE_RECV_TABLE(DT_LocalTFWeaponMediglovesData)),
	RecvPropDataTable("NonLocalTFWeaponMediglovesData", 0, 0, &REFERENCE_RECV_TABLE(DT_TFWeaponMediglovesDataNonLocal)),
#endif
END_NETWORK_TABLE()

#ifdef CLIENT_DLL
BEGIN_PREDICTION_DATA( CWeaponMedigloves  )
...
#define NUM_MEDIGLOVES_PATH_POINTS		8
...
const char *g_pszMediglovesHealSounds[] =
{
	"WeaponMedigloves.HealingWorld",		// MEDIGLOVES_CHARGE_INVULN = 0,
	"WeaponMedigloves.HealingWorld",		// MEDIGLOVES_CHARGE_CRITICALBOOST,
	"Weapon_Quick_Fix.Healing",			// MEDIGLOVES_CHARGE_MEGAHEAL,
};

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
CWeaponMedigloves::CWeaponMedigloves( void )
{
	WeaponReset();
...
// Purpose: 
//-----------------------------------------------------------------------------
CWeaponMedigloves::~CWeaponMedigloves()
{
#ifdef CLIENT_DLL
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponMedigloves::WeaponReset( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponMedigloves::Precache()
{
	BaseClass::Precache();
	PrecacheModel( "models/weapons/c_models/c_proto_backpack/c_proto_backpack.mdl" );
	PrecacheScriptSound( "WeaponMedigloves.NoTarget" );
	PrecacheScriptSound( "Weapon_Quick_Fix.Healing" );
	PrecacheScriptSound( "WeaponMedigloves.Charged" );
	...
	PrecacheScriptSound( "WeaponMedigloves_Vaccinator.Charged_tier_01");
	PrecacheScriptSound( "WeaponMedigloves_Vaccinator.Charged_tier_02");
	PrecacheScriptSound( "WeaponMedigloves_Vaccinator.Charged_tier_03");
	PrecacheScriptSound( "WeaponMedigloves_Vaccinator.Charged_tier_04");
	PrecacheScriptSound( "WeaponMedigloves.HealingDisrupt" );
	PrecacheScriptSound( "WeaponMedigloves.HealingDetachHealer" );
	PrecacheScriptSound( "WeaponMedigloves.HealingDetachTarget" );
	PrecacheScriptSound( "WeaponMedigloves.HealingDetachWorld" );
	PrecacheScriptSound( "WeaponMedigloves.HealingHealer" );
	PrecacheScriptSound( "WeaponMedigloves.HealingTarget" );
	PrecacheScriptSound( "WeaponMedigloves.HealingWorld" );
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CWeaponMedigloves::Deploy( void )
{
	if ( BaseClass::Deploy() )
	{
		m_bHolstered = false;
		...
		{
			CreateMediglovesShield();
		}

		// Resume healing self for Quick-Fix if we're still ubering and switch back to the Quick-Fix.
		if ( ( GetMediglovesType() == MEDIGLOVES_QUICKFIX ) && m_bChargeRelease && !m_bHealingSelf )
		{
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CWeaponMedigloves::Holster( CBaseCombatWeapon *pSwitchingTo )
{
	RemoveHealingTarget( true );
	m_bAttacking = false;
	m_bHolstered = true;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponMedigloves::UpdateOnRemove( void )
{
	m_bHealing = false;
	RemoveHealingTarget( true );
	m_bAttacking = false;
...
// Purpose: 
//-----------------------------------------------------------------------------
float CWeaponMedigloves::GetTargetRange( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
float CWeaponMedigloves::GetStickRange( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
float CWeaponMedigloves::GetHealRate( void )
{
	float flHealRate = m_pWeaponInfo->GetWeaponData( m_iWeaponMode ).m_nDamage;
	CALL_ATTRIB_HOOK_FLOAT_ON_OTHER( GetOwnerEntity(), flHealRate, mult_medigloves_healrate );
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CWeaponMedigloves::HealingTarget( CBaseEntity *pTarget )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CWeaponMedigloves::AllowedToHealTarget( CBaseEntity *pTarget )
{
	CTFPlayer *pOwner = ToTFPlayer( GetOwnerEntity() );
	if ( !pOwner )
		return false;
...
class CMediglovesFilter : public CTraceFilterSimple
{
public:
	CMediglovesFilter( CBaseEntity *pShooter ) : CTraceFilterSimple( pShooter, COLLISION_GROUP_WEAPON )
	{
		m_pShooter = pShooter;
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponMedigloves::MaintainTargetInSlot()
{
	CTFPlayer *pOwner = ToTFPlayer( GetOwnerEntity() );
	if ( !pOwner )
		return;
	...
	if ( flDistance < GetStickRange() )
	{
		...
		trace_t tr;
		CMediglovesFilter drainFilter( pOwner );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponMedigloves::FindNewTargetForSlot()
{
	CTFPlayer *pOwner = ToTFPlayer( GetOwnerEntity() );
	if ( !pOwner )
		return;
	...
	if ( tr.fraction != 1.0 && tr.m_pEnt )
	{
		...
		if ( pTarget )
		{
			...
			SetContextThink( &CWeaponMedigloves::HealTargetThink, gpGlobals->curtime, s_pszMediglovesHealTargetThink );
#endif
...
// Purpose:
//-----------------------------------------------------------------------------
bool CWeaponMedigloves::IsReleasingCharge( void ) const
{ 
...
// Purpose:
//-----------------------------------------------------------------------------
int CWeaponMedigloves::GetMediglovesType( void ) const 
{ 
	int iMode = 0;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponMedigloves::SetChargeLevelToPreserve( float flAmount )
{
	CTFPlayer *pOwner = ToTFPlayer( GetOwnerEntity() );
...
// Purpose:
//-----------------------------------------------------------------------------
float CWeaponMedigloves::GetMinChargeAmount( void ) const
{
	switch( GetMediglovesType() ) {
		case MEDIGLOVES_BLAST:
			return 1.f / weapon_medigloves_blast_num_chunks.GetInt();
		case MEDIGLOVES_GRAPPLE:
			return 1.f / weapon_medigloves_grapple_num_chunks.GetInt();
		case MEDIGLOVES_JUMP:
			return 1.f / weapon_medigloves_jump_num_chunks.GetInt();
		case MEDIGLOVES_TELEPORT:
			return 1.f / weapon_medigloves_teleport_num_chunks.GetInt();
		default:
			return 1.f;
	}
}
...
// Purpose:
//-----------------------------------------------------------------------------
medigloves_charge_types CWeaponMedigloves::GetChargeType( void ) const
{ 
	int iTmp = MEDIGLOVES_CHARGE_BLAST;
	CALL_ATTRIB_HOOK_INT( iTmp, set_charge_type );

	return (medigloves_charge_types)iTmp;
}

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
bool CWeaponMedigloves::IsAllowedToTargetBuildings( void )
{
	return false;
}
...
// Purpose:
//-----------------------------------------------------------------------------
bool CWeaponMedigloves::IsAttachedToBuilding( void )
{
	if ( !m_hHealingTarget )
		return false;
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponMedigloves::HealTargetThink( void )
{	
	// Verify that we still have a valid heal target.
	CBaseEntity *pTarget = m_hHealingTarget;
	if ( !pTarget || !pTarget->IsAlive() )
	{
		SetContextThink( NULL, 0, s_pszMediglovesHealTargetThink );
		return;
	}
	...
	SetNextThink( gpGlobals->curtime + 0.2f, s_pszMediglovesHealTargetThink );
}

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponMedigloves::StartHealingTarget( CBaseEntity *pTarget )
{
	CTFPlayer *pTFTarget = ToTFPlayer( pTarget );
	if ( !pTFTarget )
		return;
	...
	if( GetMediglovesType() == MEDIGLOVES_RESIST )
	{
		pTFTarget->m_Shared.AddCond( g_MediglovesResistConditions[ GetResistType() ].passiveCond, PERMANENT_CONDITION, pOwner );
		pOwner->m_Shared.AddCond( g_MediglovesResistConditions[ GetResistType() ].passiveCond, PERMANENT_CONDITION, pOwner );
	}
}
...
//-----------------------------------------------------------------------------
void CWeaponMedigloves::StopHealingOwner( void )
{
	if ( !m_bHealingSelf ) 
		return;
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponMedigloves::AddCharge( float flPercentage )
{
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponMedigloves::SubtractCharge( float flPercentage )
{
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponMedigloves::RecalcEffectOnTarget( CTFPlayer *pPlayer, bool bInstantRemove )
{
	if ( !pPlayer )
		return;
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponMedigloves::UberchargeChunkDeployed()
{
	m_nChargesReleased++;
	if( m_nChargesReleased % weapon_medigloves_resist_num_chunks.GetInt() == 0 )
	{
		CTFPlayer *pOwner = ToTFPlayer( GetOwnerEntity() );
		if ( !pOwner )
			return;
...
// Purpose:
//-----------------------------------------------------------------------------
const char *CWeaponMedigloves::GetHealSound( void ) const
{
	int iMediglovesType = GetMediglovesType();
	const char *pszRetVal = g_pszMediglovesHealSounds[iMediglovesType];
	if ( true ) // Change this later if/when diff heal sounds for diff gloves are added
	{
		...
		if ( pLocalPlayer )
		{
			...
			if ( pFiringPlayer && m_hHealingTarget.Get() )
			{
				...
				{
					pszRetVal = "WeaponMedigloves.HealingHealer";
				}
				else if ( pLocalPlayer == m_hHealingTarget.Get() )
				{
					pszRetVal = "WeaponMedigloves.HealingTarget";
				}
			}
		}
	}
...
// Purpose:
//-----------------------------------------------------------------------------
const char *CWeaponMedigloves::GetDetachSound( void ) const
{
	int iMediglovesType = GetMediglovesType();
	const char *pszRetVal = "WeaponMedigloves.HealingDetachWorld";
	if ( true ) // Change this later if/when diff heal sounds for diff gloves are added
	{
		C_TFPlayer *pLocalPlayer = C_TFPlayer::GetLocalTFPlayer();
		if ( pLocalPlayer )
		{
			...
			if ( pFiringPlayer && ( m_hHealingTarget.Get() || m_hLastHealingTarget.Get() ) )
			{
				if ( pLocalPlayer == pFiringPlayer )
				{
					pszRetVal = "WeaponMedigloves.HealingDetachHealer";
				}
				else if ( ( pLocalPlayer == m_hHealingTarget.Get() ) || ( pLocalPlayer == m_hLastHealingTarget.Get() ) )
				{
					pszRetVal = "WeaponMedigloves.HealingDetachTarget";
				}
			}
		}
	}
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponMedigloves::CreateMediglovesShield( void )
{
#ifdef GAME_DLL
	if ( m_hMediglovesShield )
		return;
	...
	m_hMediglovesShield = CTFMediglovesShield::Create( pOwner );
	if ( m_hMediglovesShield )
	{
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponMedigloves::RemoveMediglovesShield( void )
{
#ifdef GAME_DLL
	if ( m_hMediglovesShield )
	{
		m_hMediglovesShield->RemoveShield();
		m_hMediglovesShield.Set( NULL );
	}
#endif // GAME_DLL
}
...
//-----------------------------------------------------------------------------
bool CWeaponMedigloves::FindAndHealTargets( void )
{
	CTFPlayer *pOwner = ToTFPlayer( GetOwnerEntity() );
	if ( !pOwner )
		return false;
	...
	if ( pNewTarget && pNewTarget->IsAlive() )
	{
		...
		if ( !m_bChargeRelease )
		{
			...
			if ( pTFPlayer && weapon_medigloves_charge_rate.GetFloat() )
			{
				...
				// The resist medigloves has a uber charge rate
				...
				{
					CALL_ATTRIB_HOOK_FLOAT_ON_OTHER( pOwner, flChargeAmount, mult_medigloves_overheal_uberchargerate );
				}

				CALL_ATTRIB_HOOK_FLOAT_ON_OTHER( pOwner, flChargeAmount, mult_medigloves_uberchargerate );
...
#ifdef CLIENT_DLL
				// This will need to be changed if single-charge gloves are ever added
				if ( /*GetMediglovesType() == MEDIGLOVES_RESIST &&*/ prediction->IsFirstTimePredicted() )
				{
					...
					{
						const char* pzsSoundName =  CFmtStr( "WeaponMedigloves_Vaccinator.Charged_tier_0%d", nChargeLevel );
	...
	{
		float flChargeRate = weapon_medigloves_passive_charge_rate.GetFloat();
		float flChargeAmount = gpGlobals->frametime / flChargeRate;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponMedigloves::ItemHolsterFrame( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponMedigloves::DrainCharge( void )
{
	...
	if ( m_bChargeRelease )
	{
		...
		int flUberTime = weapon_medigloves_chargerelease_rate.GetFloat();
...
void CWeaponMedigloves::SubtractChargeAndUpdateDeployState( float flSubtractAmount, bool bForceDrain )
{
	CTFPlayer *pOwner = ToTFPlayer( GetOwnerEntity() );
	if ( !pOwner )
		return;
	...
	if( true )	// This assumes that all medigloves have multiple charges
	{
...
//-----------------------------------------------------------------------------
void CWeaponMedigloves::ItemPostFrame( void )
{
	CTFPlayer *pOwner = ToTFPlayer( GetOwnerEntity() );
	if ( !pOwner )
		return;
	...
	if ( AppliesModifier() )
	{
		m_DamageModifier.SetModifier( weapon_medigloves_damage_modifier.GetFloat() );
	}
#endif
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CWeaponMedigloves::Lower( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponMedigloves::RemoveHealingTarget( bool bStopHealingSelf )
{
	CTFPlayer *pOwner = ToTFPlayer( GetOwnerEntity() );
	if ( !pOwner )
		return;
...
#ifdef GAME_DLL
	int nMediglovesType = GetMediglovesType();
	...
	if ( m_hHealingTarget )
	{
		...
		if ( m_hHealingTarget->IsPlayer() )
		{
			...
			if ( pTFPlayer && pOwner )
			{
				...
				if( nMediglovesType == MEDIGLOVES_RESIST )
				{
					ETFCond cond = g_MediglovesResistConditions[ GetResistType() ].passiveCond;
...
//-----------------------------------------------------------------------------
void CWeaponMedigloves::PrimaryAttack( void )
{
	CTFPlayer *pOwner = ToTFPlayer( GetOwnerEntity() );
	if ( !pOwner )
		return;
...
#if !defined (CLIENT_DLL)
	if ( tf_medigloves_lagcomp.GetBool() )
...
//-----------------------------------------------------------------------------
void CWeaponMedigloves::SecondaryAttack( void )
{
	CTFPlayer *pOwner = ToTFPlayer( GetOwnerEntity() );
	if ( !pOwner )
		return;
	...
	float flChunkSize = GetMinChargeAmount();
	bool bDenyUse = m_flChargeLevel < flChunkSize;
...
#ifdef GAME_DLL
	// We dont want to give the user a point every time they deploy an uber with the resist medigloves.
	...
	pOwner->EmitSound( filter, pOwner->entindex(), CFmtStr( "WeaponMedigloves_Vaccinator.Charged_tier_0%d", nCurrentChunk ) );
	pOwner->EmitSound( filter, pOwner->entindex(), "WeaponMedigun_Vaccinator.InvulnerableOn"" );	
...
//-----------------------------------------------------------------------------
void CWeaponMedigloves::WeaponIdle( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponMedigloves::StopHealSound( bool bStopHealingSound, bool bStopNoTargetSound, bool bStopDetachSound )
{
	if ( bStopHealingSound && m_pHealSound )
	{
		...
		if ( !bStopDetachSound )
		{
			...
			int iIndex = entindex();
			if ( true ) // Previous conditional was ( GetMediglovesType() == MEDIGLOVES_CHARGE_INVULN ) || ( GetMediglovesType() == MEDIGLOVES_CHARGE_CRITICALBOOST ), this is a scrappy fix
			{
	...
	{
		StopSound( "WeaponMedigloves.NoTarget" );
	}
...
void CWeaponMedigloves::StopChargeEffect( bool bImmediately )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponMedigloves::ManageChargeEffect( void )
{
	C_TFPlayer *pLocalPlayer = C_TFPlayer::GetLocalTFPlayer();
	...
	if ( GetTFPlayerOwner() && bOwnerTaunting == false && m_bHolstered == false && ( m_flChargeLevel >= flMinChargeToDeploy || m_bChargeRelease == true ) )
	{
		...
		if ( m_pChargedSound == NULL )
		{
			...
			m_pChargedSound = controller.SoundCreate( filter, entindex(), "WeaponMedigloves.Charged" );
			controller.Play( m_pChargedSound, 1.0, 100 );
		}
	}
	else
...
// Input  : updateType - 
//-----------------------------------------------------------------------------
void CWeaponMedigloves::OnDataChanged( DataUpdateType_t updateType )
{
	...
	if ( m_nOldChargeResistType != m_nChargeResistType )
	{
		...
		{
			// Sound effect
			pLocalPlayer->EmitSound( "WeaponMedigloves_Vaccinator.Toggle" );	
		}
	}
	...
	if ( m_bHealing )
	{
		...
		if ( !m_pDisruptSound && pTarget && pTarget->m_Shared.InCond( TF_COND_HEALING_DEBUFF ) )
		{
			CLocalPlayerFilter filter;
			CSoundEnvelopeController &controller = CSoundEnvelopeController::GetController();
			m_pDisruptSound = controller.SoundCreate( filter, entindex(), "WeaponMedigloves.HealingDisrupt" );
	...
	else
	{
		...
		if ( IsActiveByLocalPlayer() && GetOwner() && GetOwner()->IsAlive() && m_bAttacking && GetOwner() == C_BasePlayer::GetLocalPlayer() && CanAttack() == true )
		{
			...
			{
				CLocalPlayerFilter filter;
				EmitSound( filter, entindex(), "WeaponMedigloves.NoTarget" );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponMedigloves::ClientThink()
{
	C_TFPlayer *pLocalPlayer = C_TFPlayer::GetLocalTFPlayer();
	if ( !pLocalPlayer )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponMedigloves::UpdateEffects( void )
{
	CTFPlayer *pFiringPlayer = ToTFPlayer( GetOwnerEntity() );
	if ( !pFiringPlayer )
		return;
...
//-----------------------------------------------------------------------------
void CWeaponMedigloves::UpdateMedicAutoCallers( void )
{
	// Find teammates that need healing
...
//-----------------------------------------------------------------------------
void CWeaponMedigloves::OnControlStunned( void )
{
...
// Purpose:
//-----------------------------------------------------------------------------
bool CWeaponMedigloves::EffectMeterShouldFlash( void )
{
...
// Purpose: UI Progress
//-----------------------------------------------------------------------------
float CWeaponMedigloves::GetProgress( void )
{
	CTFPlayer *pPlayer = GetTFPlayerOwner();
	if ( !pPlayer )
		return 0.f;
...
// Purpose:
//-----------------------------------------------------------------------------
float CWeaponMedigloves::GetOverHealBonus( CTFPlayer *pTFTarget )
{
	// Handle bonuses as additive, penalties as percentage...
	float flOverhealBonus = tf_max_health_boost.GetFloat() - 1.0f;
	float flMod = 1.0f;
	CALL_ATTRIB_HOOK_FLOAT( flMod, mult_medigloves_overheal_amount );
...
// Purpose:
//-----------------------------------------------------------------------------
float CWeaponMedigloves::GetOverHealDecayMult( CTFPlayer *pTFTarget )
{
	float flOverhealDecayMult = 1.f;
	CALL_ATTRIB_HOOK_FLOAT( flOverhealDecayMult, mult_medigloves_overheal_decay );
...
//-----------------------------------------------------------------------------
void CWeaponMedigloves::HookAttributes( void )
{
	CTFPlayer *pOwner = ToTFPlayer( GetOwnerEntity() );
	if ( !pOwner )
		return;
...
//-----------------------------------------------------------------------------
// entity_medigloves_shield
//-----------------------------------------------------------------------------
#define MEDIGLOVESSHIELD_THINK_CONTEXT		"CTFMediglovesShield_ShieldThink"

LINK_ENTITY_TO_CLASS( entity_medigloves_shield, CTFMediglovesShield );
PRECACHE_WEAPON_REGISTER( entity_medigloves_shield );

// Network
IMPLEMENT_NETWORKCLASS_ALIASED( TFMediglovesShield, DT_TFMediglovesShield )

BEGIN_NETWORK_TABLE( CTFMediglovesShield, DT_TFMediglovesShield )
END_NETWORK_TABLE()

BEGIN_PREDICTION_DATA( CTFMediglovesShield )
END_PREDICTION_DATA()

// Data
BEGIN_DATADESC( CTFMediglovesShield )
#ifdef GAME_DLL
...
//
//-----------------------------------------------------------------------------
CTFMediglovesShield::CTFMediglovesShield()
{
	m_nBlinkCount = 0;
#ifdef GAME_DLL
	m_pTouchLoop = NULL;
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFMediglovesShield::Spawn()
{
	BaseClass::Spawn();
	...
	SetTouch( &CTFMediglovesShield::ShieldTouch );
	SetContextThink( &CTFMediglovesShield::ShieldThink, gpGlobals->curtime, MEDIGLOVESSHIELD_THINK_CONTEXT );
#else
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFMediglovesShield::Precache()
{
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFMediglovesShield::ClientThink()
{
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFMediglovesShield::UpdateShieldPosition( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
CTFMediglovesShield *CTFMediglovesShield::Create( CTFPlayer *pOwner )
{
	if ( pOwner )
	{
		CTFMediglovesShield *pShield = static_cast< CTFMediglovesShield* >( CBaseEntity::Create( "entity_medigloves_shield", pOwner->GetAbsOrigin() + Vector( 0, 0, 60 ), pOwner->GetAbsAngles() ) );
		if ( pShield )
		{
...
// Purpose: 
//------------------------------------------------------------------------------
// bool CTFMediglovesShield::TestCollision( const Ray_t &ray, unsigned int mask, trace_t& trace )
// {
...
// Purpose:
//-----------------------------------------------------------------------------
bool CTFMediglovesShield::ShouldCollide( int collisionGroup, int contentsMask ) const
{
...
//
//-----------------------------------------------------------------------------
void CTFMediglovesShield::StartTouch( CBaseEntity *pOther )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTFMediglovesShield::ShieldTouch( CBaseEntity *pOther )
{
	if ( !pOther )
		return;
...
//
//-----------------------------------------------------------------------------
void CTFMediglovesShield::EndTouch( CBaseEntity *pOther )
{
...
//
//-----------------------------------------------------------------------------
void CTFMediglovesShield::ShieldThink( void )
{
	CTFPlayer *pOwner = ToTFPlayer( GetOwnerEntity() );
	if ( !pOwner || !pOwner->m_Shared.IsRageDraining() )
	{
	...
	SetContextThink( &CTFMediglovesShield::ShieldThink, gpGlobals->curtime + 0.1f, MEDIGLOVESSHIELD_THINK_CONTEXT );
}

//-----------------------------------------------------------------------------
//
//-----------------------------------------------------------------------------
void CTFMediglovesShield::RemoveShield( void )
{
	SetTouch( NULL );
...
//
//-----------------------------------------------------------------------------
void CTFMediglovesShield::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator )
{
...
//
//-----------------------------------------------------------------------------
int	CTFMediglovesShield::OnTakeDamage( const CTakeDamageInfo &info )
{
	if ( !InSameTeam( info.GetAttacker() ) )
	{
		CTFPlayer *pOwner = ToTFPlayer( GetOwnerEntity() );
...
// 			pOwner->EmitSound( filter, entindex(), "WeaponMedi_Shield.Protection" );
			...
			IGameEvent *event = gameeventmanager->CreateEvent( "medigloves_shield_blocked_damage" );
			if ( event )
			{
				event->SetInt( "userid", pOwner->GetUserID() );
